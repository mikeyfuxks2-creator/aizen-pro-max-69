/**
 * AIZEN PRO MAX 69 — Session 2 VERIFIER (runs as child process via fork)
 * ───────────────────────────────────────────────────────────────
 * RULES:
 *  - Completely independent WhatsApp connection
 *  - Different browser fingerprint → no clash with Session 1
 *  - Own auth folder (wa_auth_2/) → own session
 *  - Communicates with parent ONLY via IPC (process.send/on)
 *  - Handles its own reconnect (except 401/403 = wait for /pair2)
 *  - Subscribes to non-contacts via phone JID + LID JID
 */
'use strict';
const fs=require('fs'),path=require('path');

const AUTH=path.join(process.cwd(),'wa_auth_2');
const BROWSER=['Windows','Edge','22.0']; // MUST differ from Session 1
...
function log(m){console.log(`  [S2] ${m}`);}
...
async function connect(pairNumber){
    ...
    try{
        ...
        if(!s.authState.creds.registered&&pairNumber){
            ...
            log('Not paired — waiting for /pair2');
        }
        ...
        s.ev.on('connection.update',async({connection,lastDisconnect})=>{
            ...
            if(connection==='close'){
                ...
                if(code===DisconnectReason.loggedOut||code===401||code===403){
                    ...
                    // Real logout — delete and wait for /pair2
                    ...
                    setTimeout(()=>{
                        ...
                        else if(!connected) log('Not paired — waiting for /pair2');
                    },remaining+2000);
                } else {
...
if(fs.existsSync(AUTH)){
    ...
    log('No session — send /pair2 to pair');
}
